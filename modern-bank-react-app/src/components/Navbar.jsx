...
const Navbar =()=> {
    ...
    return (
        <nav className='w-full py-6 flex justify-between items-center flex-1 navbar'>
            <img src={logo} alt="hubobank" className="w-[124px] h-[32px]  cursor-pointer"/>
            
            <ul className='list-none sm:flex hidden justify-end items-center flex-1'>
                {navLinks.map((navigg, index) => (
                    <li key={navigg.id} className={` focus:text-gradient font-poppins font-normal cursor-pointer text-white text-[16px] ${index === navLinks.length - 1 ? 'mr-0' : 'mr-10'}`} key={navigg.id} >
                        <a href={`#${navigg.id}`}>
                            {navigg.title}
                        </a>
                    </li>
